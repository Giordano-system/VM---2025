...
void modificoLAR_MAR(VM *MaquinaVirtual, int operando){
    ...
    MaquinaVirtual->Registros[LAR] = (((operando & 0xFFFF) << 16) >> 16) + (MaquinaVirtual->Registros[(shiftRightLogico(operando,16)) & 0x1F]); // Armo el LAR 2 primeros bytes del segmento y los 2 ultimos el offset
...
void getMemoria(VM *MaquinaVirtual, int operando, int *valor){
    ...
    pos = logica_fisica(*MaquinaVirtual,MaquinaVirtual->Registros[(operando >> 16) & 0x1F] + (((operando & 0xFFFF) << 16) >> 16));
    *valor=0;
